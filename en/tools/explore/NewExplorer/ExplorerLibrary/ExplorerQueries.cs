    ...
    public class ExplorerQueries
    {
        ...
        public string getSpectroQuery(string specid, string objid) { 
            ...
            string iQuery = " select s.plate,s.mjd,fiberid ,s.instrument as 'Spectrograph' ,class, str(z,7,3) as 'Redshift (z)', str(zerr,10,5) as 'Redshift error' ";
                   iQuery +=" , dbo.fSpecZWarningN(zWarning) as 'Redshift flags',p.plate, p.mjd, s.fiberId ";
...
//spectro query
//select s.plate,s.mjd,fiberid ,s.instrument as 'Spectrograph' ,class, str(z,7,3) as 'Redshift (z)', str(zerr,10,5) as 'Redshift error' 
//, dbo.fSpecZWarningN(zWarning) as 'Redshift flags',p.plate, p.mjd, s.fiberId 
//,s.survey, s.programname, s.scienceprimary as 'primary', (x.nspec-1) as 'Other spec',
//s.sourcetype, str(velDisp,6,2) as 'Velocity dispersion (km/s)', str(velDispErr,7,3) as 'veldisp_error'

//--case s.survey
//--  WHEN 'sdss' THEN ( 
//--  select(dbo.fPrimtargetN(s.legacy_target1)+' '+dbo.fPrimTargetN(s.legacy_target2)+' '+dbo.fSpecialTarget1N(s.special_target1)) as targeting_flags 
//--  )
//--  WHEN 'boss' THEN (select boss_target1, ancillary_target1, ancillary_target2)
//--  WHEN 'segue1' then (select dbo.fSEGUE1target1N(segue1_target1) as segue1_targeting_flags1, dbo.fSEGUE1target2N(segue1_target2) as segue1_targeting_flags2)
//--  WHEN 'segue2' then (select dbo.fSEGUE2target1N(segue2_target1) as segue2_targeting_flags1, dbo.fSEGUE2target2N(segue2_target2) as segue2_targeting_flags2)
//--  WHEN 'apogee' then (select apogee_target1,apogee_target2 )
//--  ELSE 'The owner is another value'
//--END
//from  PlateX p ,SpecObjAll s

//join (select bestobjid, count(*) as nspec from specobjall where bestobjid=1237668296598749280 group by bestobjid) x on s.bestobjid=x.bestobjid 

//where s.specObjId=2947691243863304192
//and p.plateId=s.plateId 