...
<ol>
  ...
  <li><p>For astronomical research, use the astronomers' interface to SkyServer rather than the public interface. You will 
         ...
         interface from the main page, click on the <em>For Astronomers</em> link.</p></li>

  <li><p>An excellent to learn SQL is to modify pre-existing SQL queries. Look at the <a href="../docs/realquery.aspx">Sample SQL Queries</a> 
         on SkyServer. There is a link to them under the Help menu.</p></li>

  <li><p>Use the <a href="../../tools/chart/list.aspx" target="tools">Image List</a> tool (opens in a new window) as a sanity check on your 
   ...
   <li><p>If you're not sure how many objects a query is going to return, it's always a good idea to first do a "count" query first. A count 
    ...
    <li><p>If a query takes much longer to run than you think it should, you should try it again later to see if the problem is load on the 
           server. If it still runs slowly, look at the <b>Optimizing Queries</b> section of SkyServer's guide 
           <a href="../docs/sql_help.aspx">Using SQL with SkyServer</a>.</p></li>
    
    <li><p>Be sure to exclude invalid values (unset or uncalculated quantities) as described in the <b>Excluding Invalid Data Values</b> section 
           of SkyServer’s <a href="../docs/sql_help.aspx">Using SQL with SkyServer</a>. For example, the following query will exclude 
    ...
    <li><p>If you are running a query for photometric data, and you are searching for common parameters, consider using the <em>PhotoTag</em> table 
    ...
    <li><p>If your query returns a lot of output (more than a few thousand objects), it is generally not a good idea to 
           ...
           the <i>CSV</i> output format instead. However, you're much better off using one of the other interfaces 
           (Emacs, sqlcl or <a href="<%=globals.CasJobs%>" target="tools">CasJobs</a> - opens in new window) to get 
           large numbers of objects.</p></li>
    ...
    <li><p>If you know you want to search for both photometric and spectroscopic objects, search on the 
         ...
         <table width="100%" class="code">
           ...
           <tr>
             <td>WHERE zConf > 0.35</td>
           </tr>                      
         </table>          
                    
           </li>
           
    <li><p>You can not make a graph with Excel in a CSV file. You must first save the file as an Excel XLS file.</p></li>
</ol>
